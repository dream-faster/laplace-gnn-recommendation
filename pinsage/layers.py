...
class LinearProjector(nn.Module):
    ...
    def forward(self, ndata):
        ...
        return t.stack(projections, 1).sum(1)
...
class WeightedSAGEConv(nn.Module):
    ...
    def forward(self, g, h, weights):
        ...
        with g.local_scope():
            ...
            z = self.act(self.W(self.dropout(t.cat([n / ws, h_dst], 1))))
            z_norm = z.norm(2, 1, keepdim=True)
            z_norm = t.where(z_norm == 0, t.tensor(1.0).to(z_norm), z_norm)
            z = z / z_norm
            return z
...
class ItemToItemScorer(nn.Module):
    def __init__(self, full_graph, ntype):
        ...
        self.bias = nn.Parameter(t.zeros(n_nodes, 1))
