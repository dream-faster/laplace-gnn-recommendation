...
import pinsage.layers
import pinsage.sampler as sampler_module
import pinsage.evaluation
...
class PinSAGEModel(nn.Module):
    ...
    def get_repr(self, blocks, item_emb):
        ...
        h_item_dst = h_item_dst + item_emb(blocks[-1].dstdata[dgl.NID].cpu()).to(
            h_item_dst
        )
...
def train(dataset, args):
    ...
    # Model
    model = PinSAGEModel(g, item_ntype, textset, args.hidden_dims, args.num_layers).to(
        device
    )
